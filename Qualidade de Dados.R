# ============================================
# CÓDIGO 01: RELATÓRIO DE QUALIDADE – POR CENTRO
# Versão 19/11/25
# ============================================

suppressPackageStartupMessages({
  library(tidyverse); library(readxl); library(openxlsx)
  library(lubridate); library(janitor); library(stringr); library(readr); library(stringi)
})
...
ARQUIVO_REDCAP    <- "58214PROADIProgramaD-RandomizaoPBI_DATA_LABELS_2025-11-17_1053.csv"
...
centros_ref <- tribble(
  ~ncentro, ~nome_centro,
  1610, "PUC_PR",1611,"HCPA",1612,"UFG",1613,"HUAC",1614,"HUCAM_UFES",1615,"HUPES_UFBA",
...
if (nrow(centros_list) == 0) {
  ...
  for (i in seq_len(nrow(centros_list))) {
    ...
    n_viv_c <- viv_por_centro %>% filter(ncentro == ccode) %>% pull(n_vivanda); 
    n_viv_c <- ifelse(length(n_viv_c)==0,0,n_viv_c)
    
    n_red_c <- redcap_por_centro %>% filter(ncentro == ccode) %>% pull(n_redcap); 
    n_red_c <- ifelse(length(n_red_c)==0,0,n_red_c)
    
    comparativo_centro <- paste0("COMPARATIVO ", cnome, " (Randomizados): Vivanda (", n_viv_c, ") | REDCap (", n_red_c, ")")
    
    arq_centro <- p_(sprintf("%s_relatoriodequalidade_%s_%s.xlsx", cnome,
                             format(ref_hoje, "%b") %>% tolower(),
                             format(ref_hoje, "%Y")))
    
    # ----------------------------
    # FILTRA LINHAS JÁ RESPONDIDAS NO CONTROLE (ID + Tipo Erro + Calorias)
    # ----------------------------
    
    controle_file <- p_("Controle_inconformidades_por_centro.xlsx")
    
    if(file.exists(controle_file)){
      
      
      aba_controle <- tryCatch({
        read_excel(controle_file, sheet = cnome) %>% clean_names()
      }, error = function(e) {
        message("Não foi possível ler a aba do centro ", cnome, " no arquivo de controle.")
        return(NULL)
      })
      
      if(!is.null(aba_controle)){
        
        
        names(aba_controle) <- norm_header(names(aba_controle))
        
        
        if(all(c("id","tipo_erro","calorias","resposta") %in% names(aba_controle))){
          
          
          aba_controle <- aba_controle %>% mutate(id = as.character(id))
          
          
          linhas_respondidas <- aba_controle %>%
            filter(!is.na(resposta) & resposta != "") %>%
            select(id, tipo_erro, calorias)
          
          
          linhas_respondidas <- linhas_respondidas %>%
            mutate(
              id = as.character(id),
              tipo_erro = as.character(tipo_erro),
              calorias = as.character(calorias)
            )
          ...
          inc_c <- inc_c %>%
            mutate(
              ID = as.character(ID),
              `Tipo Erro` = as.character(`Tipo Erro`),
              Calorias = as.character(Calorias)
            )
          
          
          inc_c <- inc_c %>%
            anti_join(linhas_respondidas, by = c("ID" = "id",
                                                 "Tipo Erro" = "tipo_erro",
                                                 "Calorias" = "calorias"))
          
          
          message(sprintf("   → Centro %s: %d linhas removidas por já terem resposta", cnome,
                          nrow(inc_c_raw) - nrow(inc_c)))
        } else {
          message("Colunas obrigatórias não encontradas na aba de controle do centro ", cnome)
        }
      }
    } else {
      message("Arquivo de controle não encontrado em: ", controle_file)
    }
    
    
    wb2 <- createWorkbook()
    
    addWorksheet(wb2, "Dados")
    writeData(wb2, sheet = "Dados", inc_c, startRow = 1, startCol = 1)
    
    style_table(wb2, "Dados", inc_c, startRow = 1)
    colorize_rows_by_tipoerro(wb2, "Dados", inc_c, startRow = 1)
    apply_dropdown_por_linha(wb2, "Dados", inc_c, startRow = 1)
    
    pageSetup(wb2, sheet = "Dados", fitToWidth = 1, fitToHeight = FALSE)
    ...
    pageSetup(wb2, sheet = "Resumo", fitToWidth = FALSE, fitToHeight = FALSE)
